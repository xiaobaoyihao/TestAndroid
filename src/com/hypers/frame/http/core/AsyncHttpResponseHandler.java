...
package com.hypers.frame.http.core;
...
public abstract class AsyncHttpResponseHandler implements ResponseHandlerInterface {
    ...
    public void onStart() { 
    	if(isShowProgress){
    		progressDialog = ProgressDialog.show(context, "", "请稍候...",true); 
        ...
        switch (message.what) {
            case SUCCESS_MESSAGE:
                ...
                if (response != null && response.length >= 3) {
                	if((Integer) response[0]==200){
                		String responseBody = new String((byte[]) response[2]);
						try {
							JSONObject json = new JSONObject(responseBody);
							boolean isOk = json.optBoolean("successed");
	                		if(isOk){
	                			onSuccess((Integer) response[0], (Header[]) response[1], (byte[]) response[2]);
	                		}else{
	                			handErrorMessage(response, responseBody);
	                		}
            ...
            case FAILURE_MESSAGE:
                ...
                if (response != null && response.length >= 4) {
                	Toast.makeText(context, "网络错误("+response[0]+")", Toast.LENGTH_LONG).show();
                    onFailure((Integer) response[0], null);
                } else {
     ...
     */
    private void handErrorMessage(Object[] response,  String responseBody) throws JSONException{
    	
//    	List<ErrorMessage> messageList = JSONParse.parseErrorMessages(new JSONObject(responseBody));
//		
//		LoginResult loginResult = new LoginResult();
//		loginResult.setMessages(messageList);
//		
//		Log.i(AsyncHttpResponseHandler.class.getSimpleName(), loginResult.getErrorMessages());
//		Toast.makeText(context, loginResult.getErrorMessages(), Toast.LENGTH_SHORT).show();
//		
//		//当凭证失效、凭证逾期 、已从其他设备登录时 跳转到页面
//		if(loginResult.isGoToLoginActivity()){
//			//跳转登录页面
//			Intent intent = new Intent(context, LoginActivity.class);
//			context.startActivity(intent);
//		}
//		onFailure((Integer) response[0], loginResult.getErrorMessages());
    }
